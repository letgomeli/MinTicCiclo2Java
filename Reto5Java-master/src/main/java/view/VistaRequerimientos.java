	...
	public static void requerimiento1() throws SQLException {
		...
		try {
			for (Requerimiento_1 requerimiento : numLideresCiudad) {
				System.out.println(requerimiento.getNumLideres() + "  " + requerimiento.getCiudadResidencia());
			}
		} catch (Exception e) {
	...
	public static void requerimiento2() throws SQLException {
		...
		try {
			for (Requerimiento_2 requerimiento : materialesNoPagados) {
				System.out.println(
						requerimiento.getIdProyecto() + "  " + requerimiento.getProveedor() + "  " + requerimiento.getPagado()
								+ "  " + requerimiento.getIdMaterialConstruccion() + "  " + requerimiento.getMaterialConstruccion());
			}
		} catch (Exception e) {
			System.err.println(e);
		}
	...
	public static void requerimiento3() throws SQLException {
		...
		try {
			for (Requerimiento_3 requerimiento : lideresDebenDinero) {
				System.out.println(requerimiento.getIdLider() + "  " + requerimiento.getNombre() + "  "
						+ requerimiento.getPrimerApellido() + "  " + requerimiento.getSegundoApellido());
			}
		} catch (Exception e) {
			System.err.println(e);
		}
